...
type NodeTestLog struct {
	ID        uint64    `json:"id" gorm:"primaryKey;autoIncrement"` // 自增ID
	SubID     uint16    `json:"sub_id" gorm:"index:idx_sub_time"`   // 订阅ID
	NodeName  string    `json:"node_name"`                          // 节点名称
	Level     string    `json:"level"`                              // info / warn / error
	Message   string    `json:"message"`                            // 具体日志内容
	CreatedAt time.Time `json:"created_at" gorm:"index:idx_sub_time"` // 时间
}
